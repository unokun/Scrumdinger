...
struct SpeechRecognizer {
    ...
    func record(to speech: Binding<String>) {
        ...
        canAccess { authorized in
            ...
            guard let recognitionRequst = assistant.recognitionRequest else {
                fatalError("Unable to create request")
            }
            recognitionRequst.shouldReportPartialResults = true
            do {
                ...
                inputNode.installTap(onBus: 0, bufferSize: 1024, format: recordingFormat) { (buffer: AVAudioPCMBuffer, when: AVAudioTime) in
                    recognitionRequst.append(buffer)
                }
                relay(speech, message: "Preparing audio engine")
                audioEngine.prepare()
                try  audioEngine.start()
                assistant.recognitionTask = assistant.speechRecognizer?.recognitionTask(with: recognitionRequst) { (result, error) in
                    var isFinal = false
    ...
    private func canAccess(withHanfler handler: @escaping (Bool) -> Void) {
